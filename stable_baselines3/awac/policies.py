...
class TanhNormal(Normal):
	def log_prob_and_rsample(self) -> Tuple[th.Tensor, th.Tensor]:
		action = th.tanh(self.rsample())
		log_prob = self.log_prob(action)
		return action, log_prob

	def log_prob(self, actions: th.Tensor) -> th.Tensor:
		# It will be clipped to avoid NaN when inversing tanh
		actions = th.clamp(actions, -0.999999, 0.999999)
		gaussian_actions = th.log(1+actions)/2 - th.log(1-actions)/2

		# Log likelihood for a Gaussian distribution
		log_prob = super().log_prob(gaussian_actions)
		# Squash correction (from original SAC implementation)
		# this comes from the fact that tanh is bijective and differentiable
		log_prob -= 2.*(np.log(2.) - log_prob - nn.functional.softplus(-2.*log_prob)).sum(dim=1).reshape(-1,1)
		return log_prob

class Actor(BasePolicy):
	"""
	Actor network (policy) for AWAC.
	...
	def forward(self, obs: th.Tensor, deterministic: bool = False) -> th.Tensor:
		...
		return TanhNormal(mean_actions, th.ones_like(mean_actions)*log_std.exp())
...
class Critic(BasePolicy):
	"""
	Critic network (q-value function) for AWAC.
...
class AWACPolicy(BasePolicy):
	"""
	Policy class (with both actor and critic) for AWAC.
	...
	def __init__(self, observation_space: gym.spaces.Space,
		...
		super(AWACPolicy, self).__init__(observation_space, action_space,
										device,
...
MlpPolicy = AWACPolicy


class CnnPolicy(AWACPolicy):
	"""
	Policy class (with both actor and critic) for AWAC.
